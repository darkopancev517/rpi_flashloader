...
def loadBinToFlash(binFile):
    ...
    ledsOn([ledRed, ledGreen])
    programCmd = 'program ' + binFile + ' reset exit 0'
    command = 'sudo openocd -l temp.txt -f board/vertexcom-cm3.cfg -c "' + programCmd + '"'
    subprocess.call(command, shell=True)
    with open('temp.txt') as f:
        s = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)
        if s.find('Programming Finished') == -1:
            ledsOn([ledRed])
            ledsOff([ledGreen])
...
def main():
    while True:
        ...
        if isValidNumOfBinFiles(binFiles):
            ...
            if len(binFiles) > 0:
                print("[ERROR] multiple *.bin file in current directory, delete all")
                deleteFilesInCurrentDir(binFiles)
            ...
            binFiles = getBinFilesFromDir(currentDir)
            if len(binFiles) == 0:
                print("[ERROR] *.bin file does not exist")
